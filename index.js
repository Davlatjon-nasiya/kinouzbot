...
function createBot() {
  ...
  bot.on("polling_error", async (error) => {
    ...
    if (error.code === "ETELEGRAM" && /409 Conflict/i.test(error.message || "")) {
      console.error("❌ Telegram botning boshqa nusxasi ham polling qilmoqda.");
      console.error("❌ Eski Render service/local botni to'xtatib, faqat bitta instance qoldiring.");
...
async function startPolling() {
  ...
  try {
    ...
    console.error("⚠️ Webhook o'chirishda xato:", err.message);
  }
  ...
  try {
    ...
    console.error("❌ Polling ishga tushmadi:", err.message);

    if (!shuttingDown) {
      clearTimeout(pollingRestartTimer);
      pollingRestartTimer = setTimeout(startPolling, 10000);
    }
  }
}
...
function userMenu() {
  ...
  db.channels.forEach((c) => {
    if (c.link) buttons.push([{ text: "📢 " + c.name, url: c.link }]);
  });
...
async function getNotJoined(userId) {
  ...
  for (const channel of db.channels) {
    ...
    try {
      ...
      if (!isJoinedStatus(member.status)) notJoined.push(channel);
    } catch (err) {
...
async function sendJoinMessage(chatId) {
  ...
  if (notJoined.length === 0) {
    return bot.sendMessage(
      ...
      { reply_markup: { inline_keyboard: [[{ text: db.vipName, callback_data: "vip" }]] } }
    );
  }

  const buttons = notJoined.filter((c) => c.link).map((c) => [
    { text: "📢 " + c.name, url: c.link }
  ]);
...
function registerHandlers(bot) {
  ...
  bot.onText(/^\/admin$/, async (msg) => {
    if (msg.from.id !== ADMIN) return bot.sendMessage(msg.chat.id, "❌ Siz admin emassiz.");
  ...
  bot.on("message", async (msg) => {
    ...
    if (state?.type === "channelLink") {
      ...
      if (!/^https?:\/\/t\.me\//i.test(link)) return bot.sendMessage(id, "❌ To'g'ri Telegram link yuboring.");
      setState(id, { type: "channelId", name: state.name, link });
      return bot.sendMessage(id, "🆔 Kanal @username yoki -100... ID sini yuboring.\n\n⚠️ Bot kanalga admin qilib qo'yilgan bo'lishi kerak.");
    }

    if (state?.type === "channelId") {
      db.channels.push({ name: state.name, link: state.link, chatId: text.trim() });
      clearState(id);
      save();
    ...
    if (text === "✏️ Kanal o'zgartirish") {
      ...
      return bot.sendMessage(id, `✏️ Qaysi kanalni o'zgartiramiz?\n\n${db.channels.map((c, i) => `${i + 1}. ${c.name}`).join("\n")}`);
    }

    if (state?.type === "editNumber") {
      const n = Number(text) - 1;
      if (!Number.isInteger(n) || !db.channels[n]) return bot.sendMessage(id, "❌ Raqam noto'g'ri.");
    ...
    if (text === "👑 VIP kanal") {
      return bot.sendMessage(id, `👑 VIP KANAL\n\nNomi:\n${db.vipName}\n\nLink:\n${db.vipLink || "Kiritilmagan"}`, {
        reply_markup: { keyboard: [["✏️ VIP nomi", "🔗 VIP link"], ["🔙 Admin panel"]], resize_keyboard: true }
      });
    }
    ...
    if (text === "💳 Karta") {
      return bot.sendMessage(id, `💳 KARTA\n\n${db.card || "Karta qo'yilmagan"}`, {
        reply_markup: { keyboard: [["➕ Karta qo'shish"], ["🗑 Karta o'chirish"], ["🔙 Admin panel"]], resize_keyboard: true }
      });
    }
    ...
    if (text === "📊 Statistika") {
      return bot.sendMessage(id, `📊 STATISTIKA\n\n👥 Foydalanuvchilar: ${db.users.length}\n📢 Kanallar: ${db.channels.length}\n💳 Karta: ${db.card ? "Bor" : "Yo'q"}\n👑 VIP: ${db.vipName}`, adminMenu());
    }
    ...
    if (state?.type === "broadcast") {
      ...
      return bot.sendMessage(id, `✅ XABAR YUBORILDI\n\n📨 Yetkazildi: ${sent}\n❌ Yetkazilmadi: ${failed}\n👥 Jami: ${db.users.length}`, adminMenu());
    }
  ...
  bot.on("photo", async (msg) => {
    ...
    if (id === ADMIN) {
      ...
      return bot.sendMessage(id, `✅ RASM YUBORILDI\n\n📨 Yetkazildi: ${sent}\n❌ Yetkazilmadi: ${failed}\n👥 Jami: ${db.users.length}`, adminMenu());
    }
    ...
    const profile = msg.from.username ? `https://t.me/${msg.from.username}` : `tg://user?id=${id}`;
  ...
  bot.on("callback_query", async (q) => {
    ...
    if (q.data === "check_join") {
      try { return sendJoinMessage(chat); }
      catch (err) {
        console.error("check_join xatosi:", err.message);
        return bot.sendMessage(chat, "❌ Tekshirishda xatolik. Qayta urinib ko'ring.");
      }
    }

    if (q.data === "vip") {
      const joined = await requireJoined(id);
      if (!joined) return sendJoinMessage(chat);
      if (!db.vipLink) return bot.sendMessage(chat, "❌ VIP kanal hali sozlanmagan.");
    ...
    if (["week", "month", "year"].includes(q.data)) {
      let price = 0;
      let name = "";
      if (q.data === "week") { price = 15000; name = "1 haftalik"; }
      if (q.data === "month") { price = 50000; name = "1 oylik"; }
      if (q.data === "year") { price = 180000; name = "1 yillik"; }

      setState(id, { type: "payment", name, price });
      return bot.sendMessage(chat, `💳 TO'LOV\n\n📌 Tarif: ${name}\n\n💰 Narxi:\n${price.toLocaleString()} so'm\n\n💳 Karta:\n${db.card || "Karta hali qo'yilmagan"}\n\nTo'lovdan keyin 👇 "💰 To'ladim" tugmasini bosing.`, {
        reply_markup: { inline_keyboard: [[{ text: "💰 To'ladim", callback_data: "paid" }]] }
      });
    }

    if (q.data === "paid") {
      const state = getState(id);
      if (!state || state.type !== "payment") return bot.sendMessage(chat, "❌ Avval tarifni tanlang.");
      setState(id, { type: "receipt", name: state.name, price: state.price });
    ...
    if (q.data.startsWith("ok_")) {
      ...
      if (!db.vipLink) return bot.sendMessage(chat, "❌ VIP link hali qo'yilmagan.");
      try {
        ...
        return bot.sendMessage(chat, `❌ Foydalanuvchiga VIP link yuborilmadi: ${err.message}`);
      }
    }

    if (q.data.startsWith("no_")) {
      const userId = Number(q.data.slice(3));
      try { await bot.sendMessage(userId, "❌ To'lov rad etildi.\n\nIltimos, chekni qayta yuboring."); } catch {}
